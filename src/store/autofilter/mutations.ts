  ...
  toRaw(fieldsInfoMap).forEach((value, key) => {
    ...
    switch (fieldType) {
      ...
      case EnumTypeField.String:
        ...
        {
          const idx = values.findIndex((el: any) => el === EnumEmptyString.EmptyStringValue)
          //если есть пустая строка, тогда назвать "Пустые строки"
          if (~idx) values[idx] = EnumEmptyString.EmptyString
        }
    ...
    toRaw(fieldsInfoMap).forEach((value, key) => {
      ...
      switch (fieldType) {
        ...
        case EnumTypeField.Date:
          ...
          {
            //idx - индекс с пустой строкой
            //если есть null, тогда назвать "Пустые строки"
            const idx = values.findIndex((el: any) => el === null)
            if (~idx) values[idx] = EnumEmptyString.EmptyString
          } 
        break;
        case EnumTypeField.String:
