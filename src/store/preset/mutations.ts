...
export const mutations: MutationTree<Preset> = {
  ...
  UPDATE_SHOWED_COLUMNS_AT_COLUMN(state: Preset, showedColumn: number) {
    //удаление из скрытых столбцов определнного столбца
    const presets =  [...state.presetList]
    const idxActivePreset: number = presets.findIndex((preset: IPreset) => preset.active === true)
    const hiddenColumns: number[] = state.presetList[idxActivePreset].hiddenColumns
    const index: number = hiddenColumns.indexOf(showedColumn)
    if (~index) {
      state.presetList[idxActivePreset].hiddenColumns.splice(index, 1);
    }
  }
};