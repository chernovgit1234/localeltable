<template>
 <div class="date-type-autofilter">
    <div class="date-type-autofilter__input-body">
      <div v-if="isRangeRuleType" class="date-type-autofilter__input-range-rule-body">
        ...
        <template v-slot:append>
          <q-icon name="event" class="cursor-pointer" style="color: #424242;">
            <q-popup-proxy cover transition-show="scale" transition-hide="scale" @show="showPopup()" @hide="hidePopup()">
              <q-date v-model="dateTextRangeTypeFrom" :options="optionsByFrom">
                <div class="row items-center justify-end">
                  <q-btn v-close-popup label="Закрыть" flat />
                </div>
              </q-date>
            </q-popup-proxy>
          </q-icon>
      ...
      <div v-else>
        <q-input input-class="input-class-custom" v-model="dateText" label="дата" mask="date" :rules="['date']" outlined dense :disable="selectFromTree || isRuleTypeWhenAllDisable" >
          <template v-slot:append>
            <q-icon  name="event" class="cursor-pointer" style="color: #424242;">
              <q-popup-proxy cover transition-show="scale" transition-hide="scale" @show="showPopup()" @hide="hidePopup()">
    ...
    <div class="date-type-autofilter__settings">
      <q-btn flat icon="expand" @click="expandAll()" :disable="!treeValues.length">
        <q-tooltip class="custom-tooltip-menu" anchor="bottom middle" self="top middle" :offset="[10, 5]">
          <span class="cursor-carret_none">Раскрыть дерево</span><br/>
        </q-tooltip>
      </q-btn>
      <q-btn flat icon="vertical_align_center" @click="collapseAll()" :disable="!treeValues.length">
        <q-tooltip class="custom-tooltip-menu" anchor="bottom middle" self="top middle" :offset="[10, 5]">
          <span class="cursor-carret_none">Свернуть дерево</span><br/>
        </q-tooltip>
      </q-btn>
      <q-toggle
    ...
    <div class="date-type-autofilter__actions">
      <button :disabled="thereIsAnActiveFastFilter || !treeValues.length" class="btn-custom"  @click="applyAutofilter()" >Применить</button>
      <button :disabled="disableClearTypeDateBtn" class="btn-light-custom" @click="clearAutofilter()">Сбросить</button>
    </div>
 </div>
</template>
